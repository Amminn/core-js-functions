...
function getIdGeneratorFunction(startFrom) {
  let currentId = startFrom; // Initialize the counter in the closure

  // Return the generator function
  return function generateId() {
    const idToReturn = currentId; // Store the current ID to return
    currentId += 1; // Increment the counter for the next call
    return idToReturn; // Return the stored ID
  };
}
